...
class ValidDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, conf, root, cache_folder=None):
        """
        This function reads in all datasets to be used in validation and stores ANY relevant
        information which may be needed during validation as a list of edict()
        ...
        self.prev_num = 1 if not ('prev_num' in conf) else conf.prev_num
        self.use_3d_for_2d = ('use_3d_for_2d' in conf) and conf.use_3d_for_2d
        self.use_depth = False if not ('use_depth' in conf) else conf.use_depth

        # use cache?
        if (cache_folder is not None) and os.path.exists(os.path.join(cache_folder, 'valid_imdb.pkl')):
            logging.info('Preloading imdb.')
            imdb = pickle_read(os.path.join(cache_folder, 'valid_imdb.pkl'))

        else:
            
            # cycle through each dataset
            for dbind, db in enumerate(conf.datasets_valid):
                ...
                if db['anno_fmt'].lower() == 'kitti_det':
                    ...
                    valid_folder = os.path.join(root, db['name'], 'validation')

                    ann_folder = os.path.join(valid_folder, 'label_2', '')
                    cal_folder = os.path.join(valid_folder, 'calib', '')
                    im_folder = os.path.join(valid_folder, 'image_2', '')
                    depth_folder = os.path.join(valid_folder, 'depth_2', '')
                    ...
                    for annind, annpath in enumerate(annlist):
                        ...
                        impath_pre = os.path.join(valid_folder, 'prev_2', id + '_01' + db['im_ext'])
                        impath_pre2 = os.path.join(valid_folder, 'prev_2', id + '_02' + db['im_ext'])
                        impath_pre3 = os.path.join(valid_folder, 'prev_2', id + '_03' + db['im_ext'])
                        depthpath = os.path.join(valid_folder, 'depth_2', id + '.png')
            ...
            if cache_folder is not None:
                pickle_write(os.path.join(cache_folder, 'valid_imdb.pkl'), imdb)

        # store more information
        self.datasets_valid = conf.datasets_valid
        self.len = len(imdb)
        self.imdb = imdb

        # setup data augmentation transforms
        self.transform = Augmentation(conf, phase='validation')

        # setup data loader for this dataset
        self.loader = torch.utils.data.DataLoader(self, conf.batch_size, collate_fn=self.collate, shuffle=False)
        ...
        if len(cls_not_used) > 0:
            logging.info('Labels not used in validation.. {}'.format(cls_not_used))
    ...
    def __getitem__(self, index):
        ...
        if not self.video_det:
            ...
            # read images
            im = cv2.imread(self.imdb[index].path)
            # print(self.imdb[index].path)

            prev_num = 1 if self.prev_num is None else self.prev_num
            ...
            if prev_num >= 2:
                ...
                if im_pre2 is None:
                    im_pre2 = cv2.imread(self.imdb[index].path_pre)
            ...
            if prev_num >= 3:
                ...
                if im_pre3 is None:
                    im_pre3 = cv2.imread(self.imdb[index].path_pre2)
                    if im_pre3 is None:
                        im_pre3 = cv2.imread(self.imdb[index].path_pre)
