...
class Normalize(object):
    ...
    def __init__(self, mean, stds):
        ...
        depth_mean = [4413.160626995486]  # DORN
        depth_std = [3270.0158918863494]
        self.depth_mean = np.array(depth_mean, dtype=np.float32)
        self.depth_std = np.array(depth_std, dtype=np.float32)

    def __call__(self, image, imobj=None):
        image = image.astype(np.float32)
        if image.shape[2] % 3 == 1: # use_depth
            image[:, :, :-1] /= 255.0
            image[:, :, :-1] -= np.tile(self.mean, int(image[:, :, :-1].shape[2]/self.mean.shape[0]))
            image[:, :, :-1] /= np.tile(self.stds, int(image[:, :, :-1].shape[2]/self.stds.shape[0]))
            image[:, :, -1] -= self.depth_mean
            image[:, :, -1] /= self.depth_std
        else:
            image /= 255.0
        ...
        if random.rand() <= self.mirror_prob:
            ...
            for gtind, gt in enumerate(imobj.gts):
                ...
                if 'bbox_3d' in imobj.gts[gtind]:
                    ...
                    imobj.gts[gtind].bbox_3d[7] = coord3d[0]
