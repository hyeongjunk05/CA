...
from lib.visualize_motion import *
from lib.visualize_motion import project_3d as p3d_visual
...
def generate_located_anchors(conf, imdb, cache_folder):
    """
    ...
    # use cache?
    if (cache_folder is not None) and os.path.exists(os.path.join(cache_folder, 'located_anchors.pkl')) \
        and os.path.exists(os.path.join(cache_folder, 'anchors_2d.pkl')):

        located_anchors = pickle_read(os.path.join(cache_folder, 'located_anchors.pkl'))
        anchors = pickle_read(os.path.join(cache_folder, 'anchors_2d.pkl'))
    ...
    # generate anchors
        ...
        # presuming that anchors are initialized in "2d"
            ...
            feat_size = calc_output_size(np.array(conf.crop_size), conf.feat_stride)
            located_anchors = locate_anchors(anchors, feat_size, conf.feat_stride)[:, :4]
            # check all images
            for imind, imobj in enumerate(imdb):
                ...
                if len(imobj.gts) > 0:
                    ...
                    if gts_val.shape[0] > 0:
                        filtered_gts += np.concatenate((gts_val, gts_3d), axis=1).tolist()

            # convert to np
            filtered_gts = np.array(filtered_gts) #[N, 15] 
            #x1, y1, x2, y2, cx, cy, cz3d_2d, w3d, h3d, l3d, alpha, cx3d, cy3d, cz3d, rotY

            # expand dimensions
            located_anchors = np.concatenate((located_anchors, np.zeros([located_anchors.shape[0], 5])), axis=1)

            # bbox_3d order --> [cx3d, cy3d, cz3d, w3d, h3d, l3d, rotY]
            anchors_z3d = [[] for x in range(located_anchors.shape[0])]
            anchors_w3d = [[] for x in range(located_anchors.shape[0])]
            anchors_h3d = [[] for x in range(located_anchors.shape[0])]
            anchors_l3d = [[] for x in range(located_anchors.shape[0])]
            anchors_rotY = [[] for x in range(located_anchors.shape[0])]

            # find best matches for each ground truth
            ols = iou(located_anchors[:, 0:4], filtered_gts[:, 0:4])
            gt_target_ols = np.amax(ols, axis=0)
            gt_target_anchor = np.argmax(ols, axis=0)

            # assign each box to an anchor
            for gtind, gt in enumerate(filtered_gts):
            ...
            # update anchors
            for aind in range(0, located_anchors.shape[0]):
                ...
                if len(np.array(anchors_z3d[aind])) > 0:
                    ...
                    if conf.has_3d:
                        ...
                        located_anchors[aind, 4] = np.mean(np.array(anchors_z3d[aind]))
                        located_anchors[aind, 5] = np.mean(np.array(anchors_w3d[aind]))
                        located_anchors[aind, 6] = np.mean(np.array(anchors_h3d[aind]))
                        located_anchors[aind, 7] = np.mean(np.array(anchors_l3d[aind]))
                        located_anchors[aind, 8] = np.mean(np.array(anchors_rotY[aind]))

                # else:
                #     # raise ValueError('Non-used anchor #{} found'.format(aind))
            # located_anchors = pickle_read(os.path.join(cache_folder, 'located_anchors.pkl'))

            # update zeros value places
            located_anchors = np.reshape(located_anchors, (anchors.shape[0], feat_size[0] , feat_size[1], 9))
            org_located_anchors = located_anchors.copy()
            none_zero_region = org_located_anchors != 0
            
            for i in range(anchors.shape[0]):
                for j in range(feat_size[0]):
                    for k in range(feat_size[1]):
                        if org_located_anchors[i, j, k, 4] == 0:
                            for n in range(4, 9):
                                if n == 4 and none_zero_region[i, j, :, n].sum() != 0: # depth
                                    mean_value = org_located_anchors[i, j, :, n].sum() / none_zero_region[i, j, :, n].sum()
                                    located_anchors[i, j, k, n] = mean_value
                                else: # row has no any match, find all anchor i
                                    mean_value = org_located_anchors[i, :, :, n].sum() / none_zero_region[i, :, :, n].sum()
                                    located_anchors[i, j, k, n] = mean_value
            
            located_anchors = np.reshape(located_anchors, (-1, 9))

        if (cache_folder is not None):
            pickle_write(os.path.join(cache_folder, 'anchors_2d.pkl'), anchors)
            pickle_write(os.path.join(cache_folder, 'located_anchors.pkl'), located_anchors)

    conf.anchors = anchors
    conf.located_anchors = located_anchors
...
def compute_located_targets(gts_val, gts_ign, box_lbls, rois, fg_thresh, ign_thresh, bg_thresh_lo, bg_thresh_hi, best_thresh,
                     src_anchors, gts_3d=None):
    """
    Computes the bbox targets of a set of rois and a set
    of ground truth boxes, provided various ignore
    settings in configuration
...
@torch.no_grad()
def validation(conf, dataloader, net, criterion_det, iteration):
    loss_dict = defaultdict(list)
    logging.info(f'iteration: {iteration}, start validation ......')
    use_ego_motion = False if 'ego_motion' not in conf else conf.ego_motion
    for images, imobjs in tqdm(iter(dataloader)):
        if use_ego_motion:
            cls, prob, bbox_2d, bbox_3d, feat_size, motion, ego_motion, rois = net(images)
            det_loss, det_stats = criterion_det(images, cls, prob, bbox_2d, bbox_3d, motion, ego_motion, imobjs, feat_size)
        else:
            cls, prob, bbox_2d, bbox_3d, feat_size, motion, rois = net(images)
            det_loss, det_stats = criterion_det(images, cls, prob, bbox_2d, bbox_3d, motion, imobjs, feat_size)
            
        for stat in det_stats:
            if stat['group']=='loss' or stat['group']=='ssim loss' or stat['group']=='rgb loss' \
                or stat['group']=='motion loss' or stat['group']=='misc' or stat['group']=='ego motion loss':
                loss_dict[stat['name']].append(stat['val'].item())

    for key in loss_dict.keys():
        avg_loss = sum(loss_dict[key]) / len(loss_dict[key])
        logging.info(f'{key} average loss: {avg_loss}')
...
def im_detect_3d(im, net, rpn_conf, preprocess, p2, gpu=0, synced=False):
    ...
    imH_orig = curr_im.shape[0]
    imW_orig = curr_im.shape[1]
    
    im = [preprocess(image) for image in im]
    ...
    use_ego_motion = False if 'ego_motion' not in rpn_conf else rpn_conf.ego_motion
    if use_ego_motion:
        cls, prob, bbox_2d, bbox_3d, feat_size, motion, ego_motion, rois = net(im)
    else:
        cls, prob, bbox_2d, bbox_3d, feat_size, motion, rois = net(im)
    ...
    # motion 
    motion_mean = torch.tensor(rpn_conf.motion_mean, dtype=float)
    motion = motion[0]
    motion_x = motion[:, 0] + motion_mean[0]
    motion_y = motion[:, 1] + motion_mean[1]
    motion_z = motion[:, 2] + motion_mean[2]
    ...
    # located anchors
    # src_3d = rois[:, 4:].cuda().type(torch.cuda.FloatTensor)
    ...
    coords_3d = torch.stack((bbox_x3d, bbox_y3d, bbox_z3d[:bbox_x3d.shape[0]], \
                            ...
                            bbox_l3d[:bbox_x3d.shape[0]], bbox_ry3d[:bbox_x3d.shape[0]],\
                            motion_x[:bbox_x3d.shape[0]], motion_y[:bbox_x3d.shape[0]], motion_z[:bbox_x3d.shape[0]]), dim=1)
...
def get_prev_image(impath, im_name, prev_id):
    prev_impath = impath.replace('image_2', 'prev_2').replace(im_name, f'{im_name}_0{prev_id}')
    prev_image = cv2.imread(prev_impath)
    if prev_image is not None:
        return prev_image
    else:
        return get_prev_image(impath, im_name, prev_id-1)
...
def test_kitti_3d(dataset_test, net, rpn_conf, results_path, test_path, use_log=True):
    ...
    # init
    test_start = time()
    prev_num = 1 if not ('prev_num' in rpn_conf) else rpn_conf.prev_num
    use_depth = False if not ('use_depth' in rpn_conf) else rpn_conf.use_depth
    use_motion = True if not ('use_motion' in rpn_conf) else rpn_conf.use_motion

    for imind, impath in enumerate(imlist):
        idx = f'{imind:06d}'

        im = []
        im.append(cv2.imread(impath))
        im_name = impath.split('/')[-1].split('.')[0]

        for i in range(prev_num):
            prev_id = i + 1
            prev_im = get_prev_image(impath, im_name, prev_id)
            im.append(prev_im)
            ...
            # if score >= 0.95:
                ...
                if use_motion:
                    text_to_write += ('{} -1 -1 {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} '
                            + '{:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f}\n').format(cls, alpha, x1, y1, x2, y2, \
                                h3d, w3d, l3d, x3d, y3d, z3d, ry3d, score, x2d, y2d, motion_x, motion_y, motion_z)
                else:
                    text_to_write += ('{} -1 -1 {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} '
                            + '{:.6f} {:.6f} {:.6f} {:.6f}\n').format(cls, alpha, x1, y1, x2, y2, \
...
def inference_kitti_3d(dataset_test, net, rpn_conf, results_path, test_path, use_log=True):
    from lib.imdb_util import read_kitti_cal
    date = '2011_09_26'
    video_idx = '0015'
    image_root = f'/mnt/data/raw_kitti/{date}/{date}_drive_{video_idx}_sync/image_02/data'
    depth_root = f'/mnt/data/raw_kitti/{date}/{date}_drive_{video_idx}_sync/depth/data'
    results_root = f'/mnt/data/raw_kitti/{date}/{date}_drive_{video_idx}_sync/results/data'

    os.makedirs(results_root, exist_ok=True)

    imlist = os.listdir(image_root)
    h, w, c = cv2.imread(os.path.join(image_root, imlist[0])).shape
    ...
    test_start = time()
    prev_num = 1 if not ('prev_num' in rpn_conf) else rpn_conf.prev_num
    use_flow = False if not ('use_flow' in rpn_conf) else rpn_conf.use_flow
    use_depth = False if not ('use_depth' in rpn_conf) else rpn_conf.use_depth
    
    out = cv2.VideoWriter(f'/mnt/data/raw_kitti/{date}/{date}_drive_{video_idx}_sync/results/output_video_motion.avi', cv2.VideoWriter_fourcc(*'DIVX'), 10, (w, h))
    for idx in tqdm(range(prev_num, len(imlist))):
        filename = f'{idx:010d}.png'
        impath = os.path.join(image_root, filename)
        # visualize
        curr_image = cv2.imread(impath)
        prev_filename = f'{idx-1:010d}.png'
        prev_image = cv2.imread(os.path.join(image_root, prev_filename))

        im = []
        im.append(curr_image)
        im.append(prev_image)
        im_name = impath.split('/')[-1].split('.')[0]

        if use_depth:
            depth = cv2.imread(os.path.join(depth_root, filename), cv2.IMREAD_UNCHANGED)
            depth = depth[:, :, np.newaxis]
            im.append(depth)

        with open(f'/mnt/data/raw_kitti/{date}/calib_cam_to_cam.txt', 'r') as f:
            p2 = f.readlines()[25]

        p2 = list(map(float, p2.split()[1:]))
        p2 = np.array(p2).reshape(3, 4)
        p2 = np.concatenate([p2, np.array([0, 0, 0, 1]).reshape(1, 4)], axis=0)
        ...
        for boxind in range(0, min(rpn_conf.nms_topN_post, aboxes.shape[0])):
            ...
            if score >= 0.75:
                ...
                curr_corners_3d = p3d_visual(x3d, y3d, z3d, w3d, h3d, l3d, ry3d, 0, 0, 0)
                curr_corners_2d = get_2d_corners_from_3d(curr_corners_3d, p2)
                curr_corners_2d = curr_corners_2d.transpose()
                curr_image = draw_projected_box3d_with_motion(curr_image, curr_corners_2d, [motion_x, motion_y, motion_z],cls)
        cv2.imwrite(os.path.join(results_root, filename), curr_image)
        # input()
        out.write(curr_image)
    out.release()

def inference_synthetic(dataset_test, net, rpn_conf, results_path, test_path, use_log=True):
    from lib.imdb_util import read_kitti_cal
    image_root = '/mnt/data/KITTI/object/training/synthetic_motion/image_2/'
    prev_root = '/mnt/data/KITTI/object/training/synthetic_motion/prev_2/'
    depth_root = '/mnt/data/KITTI/object/training/synthetic_motion/depth_2/'
    ...
    for idx in tqdm(range(3768)):
        ...
        if curr_image is None:
            continue
        prev_filename = os.path.join(prev_root, filename)
        prev_image = cv2.imread(prev_filename)
        ...
        calib_path = f'/home/jsharp/M3D-RPN/data/kitti_split1/validation/calib/{idx:06d}.txt'
        ...
        file = open(os.path.join(results_root, f'{idx:06d}.txt'), 'w')
        text_to_write = ''
        for boxind in range(0, min(rpn_conf.nms_topN_post, aboxes.shape[0])):
            box = aboxes[boxind, :]
            score = box[4]
            ...
            if score >= 0.75:
                ...
                text_to_write += ('{} -1 -1 {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} '
                           + '{:.6f} {:.6f} {:.6f} {:.6f} {:.6f}\n').format(cls, alpha, x1, y1, x2, y2, \
                               h3d, w3d, l3d, x3d, y3d, z3d, ry3d, score, motion_x, motion_y, motion_z)
        file.write(text_to_write)
        file.close()        
