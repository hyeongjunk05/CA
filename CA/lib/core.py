...
def init_training_model(conf, cache_folder):
    ...
    # load adam
        ...
        step_size = 20000 if conf.step_size is None else conf.step_size
        gamma = 0.1 if conf.gamma is None else conf.gamma
    ...
    if 'cosine' in conf.lr_policy:
        ...
        scheduler = OneCycleLR(optimizer, max_lr=lr, total_steps=conf.max_iter, final_div_factor=1e4)
