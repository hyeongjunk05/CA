...
def main(argv):
    ...
    # pytorch_total_params = sum(p.numel() for p in rpn_net.parameters() if p.requires_grad)
    ...
    if conf.test_protocol.lower() == 'kitti':
        ...
        # inference_kitti_3d(conf.dataset_test, rpn_net, conf, results_path, paths.data)
        test_kitti_3d(conf.dataset_test, rpn_net, conf, results_path, paths.data)
        # inference_synthetic(conf.dataset_test, rpn_net, conf, results_path, paths.data)
