...
def Config():
    ...
    conf.datasets_valid = [{'name': 'kitti_split1', 'anno_fmt': 'kitti_det', 'im_ext': '.png', 'scale': 1}]
    ...
    conf.bbox_3d_proj_cross_correlation_lambda = 0
    conf.bbox_3d_proj_ssim_lambda = 0.05
    conf.bbox_3d_proj_rgb_lambda = 0.05
    conf.bbox_3d_proj_lambda = 0.0
    conf.bev_iou_lambda = 0
    
    conf.hill_climbing = True

    conf.pretrained = '/your_folder_path/best_model/model_50000_pkl'
    ...
    conf.prev_num = 1
    conf.video_det = True
    # conf.motion_mean = [0.033, -0.0003, 0.512]
    conf.motion_mean = [0, 0, 0]
    conf.patch_size = 32
    conf.use_flow = False
    conf.use_depth = True
    conf.dropout_rate = 0.2
